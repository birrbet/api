...
export class AuthResolver {
  ...
  login(@Context() context, @Args('input') loginInput: LoginRequest) {
    const { res } = context;
  ...
  async refreshToken(@Context() context: any, @CurrentUser() user: User) {
    ...
    res.cookie(
      'access-token',
      await this.tokenService.generateToken(TokenType.ACCESS, user.id),
      {
        httpOnly: true,
        maxAge: 1.8e6,
      },
    ...
    res.cookie(
      'refresh-token',
      await this.tokenService.generateToken(TokenType.REFRESH, user.id),
      {
        httpOnly: true,
        maxAge: 1.728e8,
      },
